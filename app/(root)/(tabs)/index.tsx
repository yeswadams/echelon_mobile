...
const Home = () => {
  ...
  const { data: featuredProperties, loading: featuredLoading } =
    useFetch<SanityPropertyListing[], Record<string, never>>({
            ...
            <View className="my-5">
              ...
              {featuredLoading ? (
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <FeaturedCard item={item} onPress={() => handleCardPress(item._id)} />
                  )}
                  keyExtractor={(item) => item._id}
                  horizontal
